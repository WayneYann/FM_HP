...
void TTransFlameSolver::ResTransFlameImpliSolver( Double *t, Double *y, Double *yPrime, Double *delta
			, int *iRes, Double */*rPar*/, int */*iPar*/ )
{
	int						k;
	int						nSpeciesIn = fSpecies->GetNSpeciesInSystem();
...
void TTransFlameSolver::WriteSootInfo( Double theTime, Double *temp, Double **Y
...
void TTransFlameSolver::CompDeltaIG( int k, Double temp )
{
	...
	for ( int i = 0; i < nSpeciesInSystem; ++i ) {
		for ( int j = 0; j < nSpeciesInSystem; ++j ) {
			fG_ij[k][i][j] = sqrt( sqrtMjOverMi[j][i] * mu[i] / mu[j] ) + 1.0;
