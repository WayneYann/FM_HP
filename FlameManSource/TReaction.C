...
#undef OPTREACTIONRATE // undefine by GB

// use TROE form of pressure dependent reactions, if undefined use expression from Peters, Rogg book
#define TROE
...
void TReaction::ComputeReactionRate( int i, Double &w, Double &k, Double *tBConc, Double density, Double *Y, Double *molarMass )
{
	...
	for ( int j = 0; j < nEducts; ++j ) {
		...
		if ( nu[j] > 1 ) {
			w *= pow( density * Y[number] / molarMass[number], nu[j] );
		}
		else {
			w *= density * Y[number] / molarMass[number];
		}
	}
	if ( fWithThirdBody[i] && !IsWithLindemann( i ) ) {
		w *= tBConc[fThirdBodyNumber->vec[i]];
