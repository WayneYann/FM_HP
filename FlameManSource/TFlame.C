...
char *TFlame::GetOutfileName( const char *name, FileType type )
{
	...
	else if ( type ==kBin ) {// PP
	  sprintf( fOutFile, "%s%s.bin", fOutputPath, name );
	}
	else {
...
void T0DFlame::UpdateThermoProps( Double *Y, Double temp, Double &pressure
				  , Double &density, EqOfState what, Double *sootMoments )
{
...
void T1DFlame::ComputeProperties( TFlameNodePtr flameNode, Double temp
